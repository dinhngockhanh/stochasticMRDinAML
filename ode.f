...
Cc*********************************************************************72
Cc
Ccc D1MACH returns double precision real machine-dependent constants.
Cc
Cc  Discussion:
Cc
Cc    D1MACH can be used to obtain machine-dependent parameters
Cc    for the local machine environment.  It is a function
Cc    with one input argument, and can be called as follows:
Cc
Cc      D = D1MACH ( I )
Cc
Cc    where I=1,...,5.  The output value of D above is
Cc    determined by the input value of I:.
Cc
...
Cc  Licensing:
Cc
Cc    This code is distributed under the GNU LGPL license. 
Cc
Cc  Modified:
Cc
Cc    25 April 2007
...
Cc  Author:
Cc
Cc    Original FORTRAN77 version by Phyllis Fox, Andrew Hall, Norman Schryer.
Cc    This FORTRAN77 version by John Burkardt.
Cc
Cc  Reference:
Cc
Cc    Phyllis Fox, Andrew Hall, Norman Schryer,
Cc    Algorithm 528:
Cc    Framework for a Portable Library,
Cc    ACM Transactions on Mathematical Software,
Cc    Volume 4, Number 2, June 1978, page 176-188.
Cc
Cc  Parameters:
Cc
Cc    Input, integer I, the index of the desired constant.
Cc
Cc    Output, double precision D1MACH, the value of the constant.
Cc
C      implicit none
C
...
C      if ( i < 1 ) then
C        write ( *, '(a)' ) ' '
C        write ( *, '(a)' ) 'D1MACH - Fatal error!'
C        write ( *, '(a)' ) '  The input argument I is out of bounds.'
C        write ( *, '(a)' ) '  Legal values satisfy 1 <= I <= 5.'
C        write ( *, '(a,i12)' ) '  I = ', i
C        d1mach = 0.0D+00
C        stop
C      else if ( i == 1 ) then
C        d1mach = 4.450147717014403D-308
C      else if ( i == 2 ) then
C        d1mach = 8.988465674311579D+307
C      else if ( i == 3 ) then
C        d1mach = 1.110223024625157D-016
C      else if ( i == 4 ) then
C        d1mach = 2.220446049250313D-016
C      else if ( i == 5 ) then
C        d1mach = 0.301029995663981D+000
C      else if ( 5 < i ) then
C        write ( *, '(a)' ) ' '
C        write ( *, '(a)' ) 'D1MACH - Fatal error!'
C        write ( *, '(a)' ) '  The input argument I is out of bounds.'
