...
*	TCGA ID of the patient
      character(len=6),parameter	:: str_ID 	= '400220'
...
*	Equilibrium hematopoietic mitotic cell numbers
      double precision,parameter	:: weight 	= 70.3D0
...
*	Number of cells in blood at primary/diagnosis
      double precision,parameter	:: diag_normal  = 2.94D0*(10.0D0**9)
      double precision,parameter	:: diag_blast  = 5.68D0*(10.0D0**10)
...
*	Time and strength of chemotherapy
     ...
     .					 7.0D0,48.0D0/),shape(CHEMO_TIME))
      double precision,parameter	:: CHEMO_STRE(1:CHEMO_TOTAL,1:2) =
     .		       reshape((/1*0.8D0,1*0.8D0,
     .				     5*0.8D0,5*0.8D0/),shape(CHEMO_TIME))
C      double precision,parameter	:: CHEMO_STRE(1:CHEMO_TOTAL,1:2) =
C     .		          reshape((/1.3D0,3.0D0,
C     .					  1.3D0,3.0D0/),shape(CHEMO_TIME))
C     	double precision,parameter	:: CHEMO_STRE(1:CHEMO_TOTAL,1:2) =
C     .		          reshape((/0.8D0,1.0D0,
C     .					  0.3D0,0.2D0/),shape(CHEMO_TIME))
*	Time until relapse
      double precision,parameter	:: T_RELAPSE	= 252.0D0
...
C      double precision,parameter	:: M	 = 10.0D0
